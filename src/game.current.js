...
async function createInvaders () {
    ...
    invaderModel = result.meshes[1];
    invaderModel .scaling.x = 0.50;
    invaderModel .scaling.y = 0.50;
    invaderModel .scaling.z = 0.50;
    invaderModel .isVisible = false;
    invaderModel .setParent (null);
    ...
    for (let i = 0; i < MAX_NUM_INVADERS; i++) {
        ...
        let invader = invaderModel.createInstance (createUniqueName("invader"));

        invader  .rotation   = new BABYLON.Vector3(0.0, Math.PI, Math.PI);
        invader  .position.y = 0;
        invader  .position.x = 0;
        invader  .position.z = 0;
        invader  .minHeight  = 0;
        invader  .maxHeight  = 0;
        invader  .materials  = invaderMaterial;
        invader  .health     = 0;
...
async function createScene () {
    ...
    //     bgm = new BABYLON.Sound("switchblade", "assets/Switchblade In Game BGM.mp3", scene, null, { loop: true, autoplay: true });
    bgm = new BABYLON.Sound("switchblade", "assets/Switchblade In Game BGM.mp3", scene, 
        function() {
    ...
    camera = new BABYLON.FreeCamera("camera1", new BABYLON.Vector3(0, 30, -30), scene);
...
//    camera.attachControl(canvas, true);
    ...
    createHUD ();
    playerHitBox = BABYLON.MeshBuilder.CreateBox("playerhitbox", 
    { height: 0.5, depth: 1, width: 2 }, 
    scene);
    let result = await BABYLON.SceneLoader.ImportMeshAsync ("", "./assets/", "Spitfire.glb", scene);
    console.log ("Player: ", player);
    playerHitBox.position.z = -14;
    playerHitBox.isVisible = false;
    player = result.meshes[0];
    player .position.y = 0.0;
    player .position.z = -14.0
    player .hits = 0;
    player .rotation   = new BABYLON.Vector3(0.0, 0.0, 0.0);
    player .scaling.x = 0.3;
    player .scaling.y = 0.3;
    player .scaling.z = 0.3;
    player .repairSince = 0;
    player .health = playerMaterial.length;
    player .materials = playerMaterial;
    player .addChild (playerHitBox);
    player .canon = createCanon (3.0);
    resetPlayer ();
...
function fireCanon (ship, dir, spd) {
    ...
    ship.canon.material = ship.materials [ship.health - 1];
